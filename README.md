...
```
Выберите функцию:
0) Y(x) = e ^ (cos x) * cos(sin(x))

         |x>0.75                        x+1
1)V(x) = |0 <= x <= 0.75        1 - pow(x,5)
         |x < 0                 x + ln(|cos(x)|)
0
Выберите действие:
...
4) Вычислить сумму максимального и минимального значений функции
3
Введите диапазон и шаг через пробел: -2 2 0,1
                                               ******
                                              *  |   *
                                            **   |    **
                                           *     |      *
